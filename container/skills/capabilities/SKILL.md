---
name: capabilities
description: Show what this NanoPieLot instance can do — installed skills, available tools, and system info. Read-only. Use when the user asks what the bot can do, what's installed, or runs /capabilities.
---

# /capabilities — System Capabilities Report

Generate a structured read-only report of what this NanoPieLot instance can do.
...
```bash
ls -1 /home/node/.github/skills/ 2>/dev/null || echo "No skills found"
```
...
- **MCP:** mcp__nanopielot__* (messaging, tasks, group management)

### 3. MCP server tools

The NanoPieLot MCP server exposes these tools (via `mcp__nanopielot__*` prefix):
