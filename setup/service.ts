...
<dict>
    ...
    <string>${projectRoot}/logs/nanopielot.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanopielot.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanopielot');
  } catch {
...
/**
 * Kill any orphaned nanopielot node processes left from previous runs or debugging.
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanopielot.service';
    ...
    unitPath = path.join(unitDir, 'nanopielot.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NanoPieLot Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nanopielot.log
StandardError=append:${projectRoot}/logs/nanopielot.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanopielot processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanopielot`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanopielot`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanopielot`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanopielot.sh');
  const pidFile = path.join(projectRoot, 'nanopielot.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanopielot.sh — Start NanoPieLot without systemd',
    ...
    '    echo "Stopping existing NanoPieLot (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoPieLot..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanopielot.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanopielot.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoPieLot started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanopielot.log"`,
  ];
