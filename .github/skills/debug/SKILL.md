...
# NanoPieLot Container Debugging
...
src/container-runner.ts               container/agent-runner/
    ...
    ├── groups/{folder}/.github/ ──> /workspace/group/.github/ (isolated per-group)
    └── (main only) project root ──> /workspace/project
```

**Important:** The container runs as user `node` with `HOME=/home/node`. Session files must be mounted to `/workspace/group/.github/` (not `/root/.github/`) for session resumption to work.

## Log Locations

| Log | Location | Content |
|-----|----------|---------|
| **Main app logs** | `logs/nanopielot.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/nanopielot.error.log` | Host-side errors |
| **Container run logs** | `groups/{folder}/logs/container-*.log` | Per-run: input, mounts, stderr, stdout |
| **Copilot session state** | `~/.github/projects/` | Copilot CLI session history |
...
### 1. "Copilot CLI process exited with code 1"
...
```bash
docker run --rm --entrypoint /bin/bash nanopielot-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash nanopielot-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
### 5. Session Not Resuming / "Copilot CLI process exited with code 1"

If sessions aren't being resumed (new session ID every time), or Copilot CLI exits with code 1 when resuming:

**Root cause:** The SDK looks for sessions at `$HOME/.github/projects/`. Inside the container, `HOME=/home/node`, so it looks at `/workspace/group/.github/projects/`.

**Check the mount path:**
```bash
# In container-runner.ts, verify mount is to /workspace/group/.github/, NOT /root/.github/
grep -A3 "Copilot session state" src/container-runner.ts
```

**Verify sessions are accessible:**
```bash
docker run --rm --entrypoint /bin/bash \
  -v ~/.config/github-copilot:/workspace/group/.github \
  nanopielot-agent:latest -c '
echo "HOME=$HOME"
ls -la $HOME/.github/projects/ 2>&1 | head -5
'
```

**Fix:** Ensure `container-runner.ts` mounts to `/workspace/group/.github/`:
```typescript
mounts.push({
  hostPath: claudeDir,
  containerPath: '/workspace/group/.github',  // NOT /root/.copilot
  readonly: false
});
```
...
```bash
docker run --rm -it --entrypoint /bin/bash nanopielot-agent:latest
```

## SDK Options Reference

The agent-runner uses these GitHub Copilot SDK options:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, Copilot CLI exits with code 1.
...
Copilot session state are stored per-group in `groups/{group}/.github/` for security isolation. Each group has its own session directory, preventing cross-group access to conversation history.

**Critical:** The mount path must match the container user's HOME directory:
- Container user: `node`
- Container HOME: `/home/node`
- Mount target: `/workspace/group/.github/` (NOT `/root/.github/`)
...
rm -rf data/sessions/{groupFolder}/.github/

# Also clear the session ID from NanoPieLot's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/nanopielot.log | tail -5
...
echo '{}' | docker run -i --entrypoint /bin/echo nanopielot-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"

echo -e "\n5. Session mount path correct?"
grep -q "/workspace/group/.github" src/container-runner.ts 2>/dev/null && echo "OK" || echo "WRONG - should mount to /workspace/group/.github/, not /root/.github/"
...
SESSIONS=$(grep "Session initialized" logs/nanopielot.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
