...
NanoClaw uses the GitHub Copilot SDK's signed-in-user flow. The login happens once via device auth and is stored in `data/copilot-auth/`, which each agent container mounts at `/home/node/.copilot`.
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common: wrong Node path (re-run step 7), missing Copilot auth (re-run step 4), missing channel credentials (re-invoke channel skill).
