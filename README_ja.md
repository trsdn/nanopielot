...
1. GitHub上で[trsdn/nanoclaw](https://github.com/trsdn/nanoclaw)をフォーク（Forkボタンをクリック）
...
その後、`/setup`を実行します。Copilot CLIがすべてを処理します：依存関係、認証、コンテナセットアップ、サービス設定。

> **注意:** `/`で始まるコマンド（`/setup`、`/add-whatsapp`など）は[Copilot CLIスキル](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/create-skills)です。通常のターミナルではなく、`copilot` CLIプロンプト内で入力してください。Copilot CLIをインストールしていない場合は、[docs.github.com/en/copilot/how-tos/use-copilot-agents/use-copilot-cli](https://docs.github.com/en/copilot/how-tos/use-copilot-agents/use-copilot-cli)から入手してください。
...
**機能追加ではなくスキル。** コードベースに機能（例：Telegram対応）を追加する代わりに、コントリビューターは`/add-telegram`のような[Copilot CLIスキル](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/create-skills)を提出し、あなたのフォークを変換します。あなたが必要なものだけを正確に実行するクリーンなコードが手に入ります。
...
- **グループごとの分離コンテキスト** - 各グループは独自の`AGENTS.md`メモリ、分離されたファイルシステムを持ち、そのファイルシステムのみがマウントされた専用コンテナサンドボックスで実行。
...
```
チャネル --> SQLite --> ポーリングループ --> コンテナ（GitHub Copilot SDK） --> レスポンス
```
...
はい。NanoClawはGitHub Copilot互換のモデルエンドポイントに対応しています。`.env`ファイルで以下の環境変数を設定してください：
...
注意：最高の互換性のため、モデルはGitHub Copilotフォーマットに対応している必要があります。
...
問題がある場合、セットアップ中にClaudeが動的に修正を試みます。それでもうまくいかない場合は、`copilot`を実行してから`/debug`を実行してください。Claudeが他のユーザーにも影響する可能性のある問題を見つけた場合は、セットアップのSKILL.mdを修正するPRを開いてください。
